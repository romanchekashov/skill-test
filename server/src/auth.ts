...
export const authenticateCurrentUserIfTokenExists = (req: Request, res: Response, next: NextFunction) => {
    // Gather the jwt access token from the request header
    const token = getToken(req);
    if (!token) return res.sendStatus(401); // if there isn't any token
...
export const logOutCurrentUser = (req: Request, res: Response, next: NextFunction) => {
